...
function nxs_widgets_customservice_geticonid() {
	$widget_name = basename(dirname(__FILE__));
	return "nxs-icon-publicrelations";
}

// Setting the widget title
function nxs_widgets_customservice_gettitle() {
	return nxs_l18n__("Custom Service", "nxs_td");
}

// Unistyling
function nxs_widgets_customservice_getunifiedstylinggroup() {
	return "customservicewidget";
}

// Unicontent
function nxs_widgets_customservice_getunifiedcontentgroup() {
	return "customservicewidget";
}
...
function nxs_widgets_customservice_home_getoptions() 
{
	$options = array
	(
		"sheettitle" 		=> "Custom Service",
		"sheeticonid" 		=> nxs_widgets_customservice_geticonid(),
		...
		"unifiedstyling" 	=> array ("group" => nxs_widgets_customservice_getunifiedstylinggroup(),),
		"unifiedcontent" 	=> array ("group" => nxs_widgets_customservice_getunifiedcontentgroup(),),
...
function nxs_widgets_customservice_render_webpart_render_htmlvisualization($args) 
{
	// Importing variables
	extract($args);
	...
	else
	{
		...
		if (isset($unistyle) && $unistyle != "") 
		{
			...
			$unistyleproperties = nxs_unistyle_getunistyleproperties(nxs_widgets_customservice_getunifiedstylinggroup(), $unistyle);
		...
		{
			// blend unistyle properties
			$unicontentproperties = nxs_unicontent_getunicontentproperties(nxs_widgets_customservice_getunifiedcontentgroup(), $unicontent);
...
function nxs_widgets_customservice_initplaceholderdata($args)
{
	extract($args);
	...
	$unistylegroup = nxs_widgets_customservice_getunifiedstylinggroup();
	$args = nxs_unistyle_blendinitialunistyleproperties($args, $unistylegroup);
	
	// current values as defined by unicontent prefail over the above "default" props
	$unicontentgroup = nxs_widgets_customservice_getunifiedcontentgroup();
