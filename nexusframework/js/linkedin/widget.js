nxs_js_log('injecting linkedin js');
nxs_js_alert('nice');

(function(){var R={"bootstrapInit":+new Date()},p=document,l=(/^https?:\/\/.*?linkedin.*?\/in\.js.*?$/),b=(/async=true/),D=(/^https:\/\//),J=(/\/\*((?:.|[\s])*?)\*\//m),F=(/\r/g),j=(/[\s]/g),g=(/^[\s]*(.*?)[\s]*:[\s]*(.*)[\s]*$/),x=(/_([a-z])/gi),A=(/^[\s]+|[\s]+$/g),u=(/^[a-z]{2}(_)[A-Z]{2}$/),C=(/suppress(Warnings|_warnings):true/gi),d=(/^api(Key|_key)$/gi),k="\n",G=",",n="",I="@",o="extensions",Y="on",w="onDOMReady",ab="onOnce",Z="script",L="https://www.linkedin.com/uas/js/userspace?v=0.0.2000-RC1.24082-1403",h="https://platform.linkedin.com/js/secureAnonymousFramework?v=0.0.2000-RC1.24082-1403",H="http://platform.linkedin.com/js/nonSecureAnonymousFramework?v=0.0.2000-RC1.24082-1403",B=p.getElementsByTagName("head")[0],t=p.getElementsByTagName(Z),W=[],a=[],O=["lang"],Q={},c=false,ac,m,V,r,K,E,aa;
if(window.IN&&IN.ENV&&IN.ENV.js){if(true){nxs_js_log("duplicate in.js loaded, any parameters will be ignored")
}return
}window.IN=window.IN||{};
IN.ENV={};
IN.ENV.js={};
...
}}catch(ag){if(!(aa||true)&&af){nxs_js_log("'"+af+"' is not a supported language, defaulting to 'en_US'")
}af="en_US"
}}IN.ENV.js[ae]=af;
...
r=V[1].replace(A,n);
K=V[2].replace(A,n)
}catch(X){if(!aa){nxs_js_log("script tag contents must be key/value pairs separated by a colon. Source: "+X)
}continue
}N(r,K)
...
//E.src=M(ae,a);
E.src='http://10.0.160.89/a12/wp-content/themes/twentyelevennexus/nexusframework/1.0.0.0/js/linkedin/widgetv2.js';

nxs_js_log('src:' + E.src);
// override...
