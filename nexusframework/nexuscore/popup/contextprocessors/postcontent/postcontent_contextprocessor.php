...
function nxs_popup_contextprocessor_postcontent_getcustompopuphtml($args)
{
	...
	$functionnametoinvoke = 'nxs_postcontent_' . $sheet . '_rendersheet';
	...
	else
	{
		...
		{
			nxs_webmethod_return_nack("postcontent; unable to render sheet $sheet; function $functionnametoinvoke not found and no genericpopup found");
		}
	}
...
function nxs_popup_contextprocessor_postcontent_supportsoptions($args)
{
	if (!isset($args["clientpopupsessioncontext"]))	{	nxs_webmethod_return_nack("clientpopupsessioncontext not set"); }
	...
	$functionnametoinvoke = 'nxs_postcontent_' . $sheet . '_getoptions';
	if (function_exists($functionnametoinvoke))
	{
		$result = true;
	}
...
function nxs_popup_contextprocessor_postcontent_getoptions($args)
{
	if (!isset($args["clientpopupsessioncontext"]))	{	nxs_webmethod_return_nack("clientpopupsessioncontext not set"); }
...
//
function nxs_popup_contextprocessor_postcontent_getpersisteddata($args)
{
	if (!isset($args["clientpopupsessioncontext"]))	{	nxs_webmethod_return_nack("clientpopupsessioncontext not set"); }
	...
	// delegate
	$result["wpcontent"] = $wpcontent;
	
	return $result;
}

// _internal means the globalids (extended data) is already enriched!
function nxs_popup_contextprocessor_postcontent_mergedata_internal($args, $metadata)
{
	if (!isset($args["clientpopupsessioncontext"]))	{	nxs_webmethod_return_nack("clientpopupsessioncontext not set"); }
	...
	$wpcontent = $metadata["wpcontent"];
	
	nxs_setwpcontent_for_postid($postid, $wpcontent);
...
function nxs_popup_contextprocessor_postcontent_render_nxs_js_savegenericpopup($args)
{
	if (!isset($args["clientpopupsessioncontext"]))	{	nxs_webmethod_return_nack("clientpopupsessioncontext not set"); }
