...
function nxs_popup_contextprocessor_rowscontainer_getcustompopuphtml($args)
{
	...
	else
	{
		...
		{
			nxs_webmethod_return_nack("rowscontainer context; unable to render sheet $sheet; function $functionnametoinvoke not found and no genericpopup found");
		}
	}
...
function nxs_popup_contextprocessor_rowscontainer_supportsoptions($args)
{
	if (!isset($args["clientpopupsessioncontext"]))	{	nxs_webmethod_return_nack("clientpopupsessioncontext not set"); }
	...
	// load row functions	
	$filetobeincluded = NXS_FRAMEWORKPATH . "/nexuscore/rowscontainer/rowscontainer.php";
	require_once($filetobeincluded);
	
	$functionnametoinvoke = 'nxs_rowscontainer_' . $sheet . '_getoptions';
	if (function_exists($functionnametoinvoke))
	{
		$result = true;
	}
...
function nxs_popup_contextprocessor_rowscontainer_getoptions($args)
{
	if (!isset($args["clientpopupsessioncontext"]))	{	nxs_webmethod_return_nack("clientpopupsessioncontext not set"); }
...
//
function nxs_popup_contextprocessor_rowscontainer_getpersisteddata($args)
{
	if (!isset($args["clientpopupsessioncontext"]))	{	nxs_webmethod_return_nack("clientpopupsessioncontext not set"); }
...
function nxs_popup_contextprocessor_rowscontainer_mergedata_internal($args, $metadata)
{
	if (!isset($args["clientpopupsessioncontext"]))	{	nxs_webmethod_return_nack("clientpopupsessioncontext not set"); }
...
function nxs_popup_contextprocessor_rowscontainer_render_nxs_js_savegenericpopup($args)
{
	if (!isset($args["clientpopupsessioncontext"]))	{	nxs_webmethod_return_nack("clientpopupsessioncontext not set"); }
