...
modifications:
- top condition removed
- no delete when file is finished (as file is in theme itself)
- no user interaction
- added require_once (imports) of taxonomy, import, post, image.php
*/

/** Display verbose errors */
define('NXS_DEFINE_IMPORT_DEBUG', true);
define('NXS_DEFINE_IMPORTERID', 'nxsimporter');
 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
class NXS_importer extends WP_Importer {
	 ...
	 */
	function import_end() {
		// in original importer, the id refers to the post pointing to the input source (XML) file
		// since the file is part of the theme we won't delete it
		...
		// todo: instead of invoking import_end we could here do specific implementation for our theme
		do_action( 'nxsimporter_end' );
	}
	...
	function handle_upload() {
		...
		$file = array();
		$file["file"] = ; // "/opt/bitnami/apps/wordpress/htdocs/wp-content/blogs.dir/13/files/2012/10/resources.xml_1.txt" 
	...
	*/
	...
	function import_options() {
		$j = 0;
?>
<form action="<?php echo admin_url( 'admin.php?import=' . NXS_DEFINE_IMPORTERID. '&amp;step=2' ); ?>" method="post">
				...
				foreach ( $post['postmeta'] as $meta ) {
					...
					if ($scan == true)
					{
						...
						else if ($key == "nxs_core")
						{
							...
							if (nxs_stringcontains($x, "\r\n"))
							{
								echo "SUCKERR!";	
							}
							
							
							//error_log("VOLGENDE REGEL");
							//error_log($x);
							//error_log("VORIGE REGEL");
							
							$file = "/opt/bitnami/apps/wordpress/htdocs/wp-content/gj.txt";
							file_put_contents($file, $x, FILE_APPEND | LOCK_EX);
														
							echo "oooooo here we go!";
							
							/*							
							if (nxs_stringcontains($x, "button_sca?le"))
							{
								echo "ALSJEMENOU!";
								die();
							}
...
function nxs_importer_init() 
{
	$GLOBALS['nxs_importer'] = new NXS_importer();
	register_importer(NXS_DEFINE_IMPORTERID, 'nxsimportername', 'nxsimporterdescription', array($GLOBALS['nxs_importer'], 'dispatch'));
}
add_action('admin_init', 'nxs_importer_init');
