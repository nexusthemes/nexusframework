...
function nxs_site_newtemplate_getoptions($args)
{
	...
	else if ($lp_step == "4")
	{
		...
		require_once(NXS_FRAMEWORKPATH . '/nexuscore/template/nxs-template.php');
		$args = array
		(
			"postid" => $lp_id,
			"overridetitle" => $lp_name,
		);
		$createcontentresult = nxs_site_createcontent($args);
		
		$url = "todo"; // nxs_geturl_for_globalid($nxs_site_content["semanticglobalidmapping"]["ROOTSOURCE"]);
