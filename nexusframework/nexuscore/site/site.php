...
function nxs_site_createcontent($args)
{
	...
	// map any semantic globalid to the value it should have (for example "HOME", or "ROOTSOURCE"	
	$issemantic = false;
	...
	// its not here yet; mark it as newly created during this session as thats what we will do right now
	// this is done to prevent the item from being recreated endless times; we only want one
	...
	else if (in_array($posttype, array("nxs_templatepart", "nxs_genericlist", "page")))
	{
		...
		if ($posttype == "nxs_templatepart")
		{
			if ($contentcurrentpost["nxs_semanticlayout"] == "")
			{
				$postmetas["nxs_semanticlayout"] = "landingpage";
			}
		}
			...
			foreach ($placeholderids as $placeholderid)
			{
				...
				foreach ($widgetmetadata as $key => $val)
				{
					if (nxs_stringendswith($key, "_globalid"))	 // p.e. image_imageid_globalid
					{
						...
						$createresult = nxs_site_createcontent($recursiveargs);
...
function nxs_site_newlandingpage_getoptions($args)
{
	...
	if ($lp_step == "1" || $lp_step == "")
	{
		...
		{
			$title = $tagmeta["title"];
			$tagfilterhtml .= "<a href='#' onclick='nxs_js_lp_filtertags(\"{$tag}\"); return false;'>{$title}</a><br />";	
		}
	...
	else if ($lp_step == "2")
	{
		...
		<script>
			...
			function nxs_js_lp_next()
			{
				// locate all shortscope input elements
				jQ_nxs(".nxs-persistmode-shortscope").each
				(
					function(index)
					{
						// store them
						var shortscopeid = jQ_nxs(this).attr('id');
						var shortscopevalue = jQ_nxs("#" + shortscopeid).val();
						// nxs_js_log("index: " + index + " | identifier;" + shortscopeid + " | value:" + shortscopevalue);
						nxs_js_popup_setshortscopedata(shortscopeid, shortscopevalue);
					}
				);
		...
		(
			...
			(
				...
				(
					...
					"label"			=> "",
					"type" 				=> "custom",
					"customcontent" => "<a class='nxsbutton nxs-float-right' href='#' onclick='nxs_js_lp_next();'>Next</a>",
				),
				array
				(
					"id" 			=> "custom",
	...
	else if ($lp_step == "3")
	{
		...
		$args = array
		(
			"currentpostglobalid" => "ROOTSOURCE",
			"preventendlessloop" => 20,	// max 100 posts to be created
			"overridetitle" => $lp_name, 
		);
	...
	else
	{
		...
		$result = array
		(
			"sheettitle" => nxs_l18n__("Unsupported step {$lp_step}", "nxs_td"),
			"fields" => array
			(
				array(
					"id" 			=> "custom",
