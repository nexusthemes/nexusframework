...
function nxs_widgets_woothankyou_geticonid() {
	$widget_name = basename(dirname(__FILE__));
	return "nxs-icon-" . $widget_name;
}

// Setting the widget title
function nxs_widgets_woothankyou_gettitle() {
	return nxs_l18n__("woothankyou", "nxs_td");
}

// Unistyle
function nxs_widgets_woothankyou_getunifiedstylinggroup() {
	return "woothankyouwidget";
}

function nxs_woo_aff_shareasale_merchantid_impl($shareasalemerchantid)
{
	global $nxs_glb_affiliate;
	return $nxs_glb_affiliate; 
}
...
function nxs_widgets_woothankyou_home_getoptions($args) 
{
	// CORE WIDGET OPTIONS

	$options = array
	(
		"sheettitle" => nxs_widgets_woothankyou_gettitle(),
		"sheeticonid" => nxs_widgets_woothankyou_geticonid(),
		"sheethelp" => nxs_l18n__("http://nexusthemes.com/woothankyou-widget/"),		
		"unifiedstyling" => array
		(
			"group" => nxs_widgets_woothankyou_getunifiedstylinggroup(),
		),
		"fields" => array
		(
			array(
				"id" 				=> "shareasalemerchantid_text",
				"type" 				=> "input",
				"label" 			=> nxs_l18n__("Shareasalemerchantid", "nxs_td"),
				"placeholder"		=> "shareasalemerchantid",
			),
		)
	);
...
function nxs_widgets_woothankyou_render_webpart_render_htmlvisualization($args) 
{	
	// Importing variables
	extract($args);
	...
	if (isset($unistyle) && $unistyle != "") {
		...
		$unistyleproperties = nxs_unistyle_getunistyleproperties(nxs_widgets_woothankyou_getunifiedstylinggroup(), $unistyle);
	...
	) 
	{
 	...
 	// Image
	if ($image_imageid != "") {     
		// Core WP function returns ID ($woothankyou_id), size of image (thumbnail, medium, large or full)
	...
	if ($image_imageid != "") {
		$woothankyou = '
			<div class="woothankyou-image" style="max-width: ' . $imagewidth . ';  ' . $alignment_image . ' ">
	...
	if ($shareasalemerchantid_text != "")
	{
		global $nxs_glb_affiliate;
		$nxs_glb_affiliate = $shareasalemerchantid_text;
		add_filter("nxs_woo_aff_shareasale_merchantid", "nxs_woo_aff_shareasale_merchantid_impl");
	}
...
function nxs_widgets_woothankyou_initplaceholderdata($args)
{
	extract($args);
	...
	$unistylegroup = nxs_widgets_woothankyou_getunifiedstylinggroup();
