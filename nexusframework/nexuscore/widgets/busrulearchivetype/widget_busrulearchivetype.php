...
function nxs_widgets_busrulearchivetype_geticonid() {
	$widget_name = basename(dirname(__FILE__));
	return "nxs-icon-books";
}

// Setting the widget title
function nxs_widgets_busrulearchivetype_gettitle() {
	return nxs_l18n__("Archive posttype", "nxs_td");
}

function nxs_widgets_busrulearchivetype_getposttypes()
{
	$result = array();
...
function nxs_widgets_busrulearchivetype_home_getoptions($args) 
{
	// CORE WIDGET OPTIONS
	
	$options = array
	(
		"sheettitle" => nxs_widgets_busrulearchivetype_gettitle(),
		"sheeticonid" => nxs_widgets_busrulearchivetype_geticonid(),
		...
		(
			...
			array(
				...
				"dropdown" 		=> nxs_widgets_busrulearchivetype_getposttypes(),
			),	
			array( 
...
function nxs_widgets_busrulearchivetype_render_webpart_render_htmlvisualization($args) 
{	
	// Importing variables
	extract($args);
	...
	*/
	...
	{
		$post_type_archive_link = get_post_type_archive_link($filter_posttype);
		
		if ($post_type_archive_link == "")
		{
			$filteritemshtml = $filter_posttype . " archive";
		}
		else
		{
			$filteritemshtml = "<a target='_blank' href='" . $post_type_archive_link . "'>" . $filter_posttype . " archive</a>";
		}
		
		nxs_widgets_busrule_pagetemplate_renderrow(nxs_widgets_busrulearchivetype_geticonid(), $filteritemshtml, $mixedattributes);
	}
...
function nxs_widgets_busrulearchivetype_initplaceholderdata($args)
{
	extract($args);
...
function nxs_busrule_busrulearchivetype_process($args, &$statebag)
{
	$result = array();
	$result["result"] = "OK";

	$metadata = $args["metadata"];
	$filter_posttype = $metadata["filter_posttype"];
	
	$is_post_type_archive = is_post_type_archive($filter_posttype);
	if ($is_post_type_archive)
	{
		...
		foreach($sitewideelements as $currentsitewideelement)
		{
			...
			if ($selectedvalue == $filter_posttype)
			{
				// skip
			} 
