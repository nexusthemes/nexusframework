...
function nxs_widgets_woomessages_geticonid() {
	$widget_name = basename(dirname(__FILE__));
	return "nxs-icon-" . $widget_name;
}

// Setting the widget title
function nxs_widgets_woomessages_gettitle() {
	return nxs_l18n__("woomessages", "nxs_td");
}

// Unistyle
function nxs_widgets_woomessages_getunifiedstylinggroup() {
	return "woomessageswidget";
}
...
function nxs_widgets_woomessages_home_getoptions($args) 
{
	// CORE WIDGET OPTIONS

	$options = array
	(
		"sheettitle" => nxs_widgets_woomessages_gettitle(),
		"sheeticonid" => nxs_widgets_woomessages_geticonid(),
		"sheethelp" => nxs_l18n__("http://nexusthemes.com/woomessages-widget/"),		
		"unifiedstyling" => array
		(
			"group" => nxs_widgets_woomessages_getunifiedstylinggroup(),
		),
		"fields" => array
		(
			// woomessages
...
function nxs_widgets_woomessages_render_webpart_render_htmlvisualization($args) 
{	
	// Importing variables
	extract($args);
	...
	if (isset($unistyle) && $unistyle != "") {
		...
		$unistyleproperties = nxs_unistyle_getunistyleproperties(nxs_widgets_woomessages_getunifiedstylinggroup(), $unistyle);
	...
	if ($image_imageid != "") {
		$woomessages = '
			<div class="woomessages-image" style="max-width: ' . $imagewidth . ';  ' . $alignment_image . ' ">
	...
	else 
	{
		...
		{
			echo "WooCommerce notifications";
		}
		else
		{
		...
		foreach ($woocommerce->get_messages() as $currentmessage)
		{
			echo $currentmessage;
			echo "<br />";
		}
		foreach ($woocommerce->get_errors() as $currentmessage)
		{
			echo $currentmessage;
			echo "<br />";
		}
...
function nxs_widgets_woomessages_initplaceholderdata($args)
{
	extract($args);
	...
	$unistylegroup = nxs_widgets_woomessages_getunifiedstylinggroup();
