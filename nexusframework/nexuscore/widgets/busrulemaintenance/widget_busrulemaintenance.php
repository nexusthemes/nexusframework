...
function nxs_widgets_busrulemaintenance_geticonid() {
	$widget_name = basename(dirname(__FILE__));
	return "nxs-icon-construction";
}

// Setting the widget title
function nxs_widgets_busrulemaintenance_gettitle() {
	return nxs_l18n__("Maintenance mode", "nxs_td");
}
...
function nxs_widgets_busrulemaintenance_home_getoptions($args) 
{
	$options = array
	(
		"sheettitle" => nxs_widgets_busrulemaintenance_gettitle(),
		"sheeticonid" => nxs_widgets_busrulemaintenance_geticonid(),
		"fields" => array
		(
			//	
		)
...
function nxs_widgets_busrulemaintenance_render_webpart_render_htmlvisualization($args) 
{	
	// Importing variables
	extract($args);
...
function nxs_widgets_busrulemaintenance_initplaceholderdata($args)
{
	extract($args);
...
function nxs_busrule_busrulemaintenance_process($args, &$statebag)
{
	$result = array();
	$result["result"] = "OK";
	...
	if (!is_user_logged_in() && !nxs_iswploginpage() && nxs_issiteinmaintenancemode())
	{
		$result["ismatch"] = "true";
		...
		// concatenate the modeluris and modelmapping (do NOT yet evaluate them; this happens in stag	e 2, see #43856394587)
...
function nxs_dataprotection_nexusframework_widget_busrulemaintenance_getprotecteddata($args)
{
