...
function nxs_widgets_busrulesearch_geticonid() {
	$widget_name = basename(dirname(__FILE__));
	return "nxs-icon-search";
}

// Setting the widget title
function nxs_widgets_busrulesearch_gettitle() {
	return nxs_l18n__("Search results", "nxs_td");
}
...
function nxs_widgets_busrulesearch_home_getoptions($args) 
{
	// CORE WIDGET OPTIONS
	
	$options = array
	(
		"sheettitle" => nxs_widgets_busrulesearch_gettitle(),
		"sheeticonid" => nxs_widgets_busrulesearch_geticonid(),
		"fields" => array
		(
...
function nxs_widgets_busrulesearch_render_webpart_render_htmlvisualization($args) 
{	
	// Importing variables
	extract($args);
...
function nxs_widgets_busrulesearch_initplaceholderdata($args)
{
	extract($args);
...
function nxs_busrule_busrulesearch_process($args, &$statebag)
{
	$result = array();
	$result["result"] = "OK";
...
function nxs_dataprotection_nexusframework_widget_busrulesearch_getprotecteddata($args)
{
