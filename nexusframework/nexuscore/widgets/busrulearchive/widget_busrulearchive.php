...
function nxs_widgets_busrulearchive_geticonid() {
	$widget_name = basename(dirname(__FILE__));
	return "nxs-icon-books";
}

// Setting the widget title
function nxs_widgets_busrulearchive_gettitle() {
...
function nxs_widgets_busrulearchive_home_getoptions($args) 
{
	// CORE WIDGET OPTIONS
	
	$options = array
	(
		"sheettitle" => nxs_widgets_busrulearchive_gettitle(),
		"sheeticonid" => nxs_widgets_busrulearchive_geticonid(),
		"fields" => array
		(
		) 
	);
...
function nxs_widgets_busrulearchive_render_webpart_render_htmlvisualization($args) 
{	
	// Importing variables
	extract($args);
	...
	*/
	...
	{
		$filteritemshtml = nxs_l18n__("Any archive", "nxs_td");
		nxs_widgets_busrule_pagetemplate_renderrow(nxs_widgets_busrulearchive_geticonid(), $filteritemshtml, $mixedattributes);
	} 
...
function nxs_widgets_busrulearchive_initplaceholderdata($args)
{
	extract($args);
...
function nxs_busrule_busrulearchive_process($args, &$statebag)
{
	$result = array();
	$result["result"] = "OK";
...
function nxs_dataprotection_nexusframework_widget_busrulearchive_getprotecteddata($args)
{
