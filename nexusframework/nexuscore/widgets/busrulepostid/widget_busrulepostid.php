...
function nxs_widgets_busrulepostid_geticonid() {
	$widget_name = basename(dirname(__FILE__));
	return "nxs-icon-file2";
}

// Setting the widget title
function nxs_widgets_busrulepostid_gettitle() {
	return nxs_l18n__("Specific post or page", "nxs_td");
}
...
function nxs_widgets_busrulepostid_home_getoptions($args) 
{
	// CORE WIDGET OPTIONS
...
function nxs_widgets_busrulepostid_render_webpart_render_htmlvisualization($args) 
{	
	// Importing variables
	extract($args);
...
function nxs_widgets_busrulepostid_initplaceholderdata($args)
{
	extract($args);
...
function nxs_busrule_busrulepostid_process($args, &$statebag)
{
	$result = array();
	$result["result"] = "OK";
...
function nxs_dataprotection_nexusframework_widget_busrulepostid_getprotecteddata($args)
{
