...
function nxs_widgets_busruleauthentication_geticonid() {
	$widget_name = basename(dirname(__FILE__));
	return "nxs-icon-user";
}

// Setting the widget title
function nxs_widgets_busruleauthentication_gettitle() {
	return nxs_l18n__("User authentication", "nxs_td");
}
...
function nxs_widgets_busruleauthentication_home_getoptions($args) 
{
	// CORE WIDGET OPTIONS
	
	$options = array
	(
		"sheettitle" => nxs_widgets_busruleauthentication_gettitle(),
		"sheeticonid" => nxs_widgets_busruleauthentication_geticonid(),
		"fields" => array
		(
			array( 
			...
			array( 
				"id" 				=> "filter_type",
				"type" 				=> "select",
				"label" 			=> nxs_l18n__("Authentication type", "nxs_td"),
				"dropdown" 			=> array(
					"authenticated"=>nxs_l18n__("Authenticated users", "nxs_td"),
					"anonymous"=>nxs_l18n__("Anonymous users", "nxs_td"), 
				),
			),
...
function nxs_widgets_busruleauthentication_render_webpart_render_htmlvisualization($args) 
{	
	// Importing variables
	extract($args);
	...
	*/
	...
	{
		$filteritemshtml = $filter_type;
		nxs_widgets_busrule_pagetemplate_renderrow(nxs_widgets_busruleauthentication_geticonid(), $filteritemshtml, $mixedattributes);
	} 
...
function nxs_widgets_busruleauthentication_initplaceholderdata($args)
{
	extract($args);
...
function nxs_busrule_busruleauthentication_process($args, &$statebag)
{
	$result = array();
	$result["result"] = "OK";
	...
	else if ($filter_type == "authenticated")
	{
		if (is_user_logged_in())
		{
			$result["ismatch"] = "true";
...
function nxs_dataprotection_nexusframework_widget_busruleauthentication_getprotecteddata($args)
{
